...
PINE Features can also be set specifically only for Ape Escape 3.

1. Under `Tools`, check `Show Advanced Settings`.
2. Right-click the Game Entry
3. Click `Properties`
4. In the `Advanced Tab`, under the `PINE Settings` section, check `Enabled`
...
When PINE is set as a game-specific property, the PINE Slot can also be set specifically for the game, allowing for easy separation of PINE Slots per game when playing multiple games that also uses PINE, such as other PS2 games under PCSX2, or PS3 games under RPCS3.
...
`PINE Settings` > `Slot`. Make sure to also check if PINE has different settings set as game-specific property by right-clicking the game and selecting `Properties`, as game-specific settings take precedence over global settings. If the slot the client is looking for and the slot in the PCSX2 settings are different, either change
