...
def can_boost_jump(state: CollectionState, player: int):
    return state.has_any([Itm.gadget_net.value, Itm.gadget_club.value], player) and state.has_from_list_unique(
        [
            Itm.gadget_net.value,
            Itm.gadget_club.value,
...
def can_boost_fly(state: CollectionState, player: int):
    return state.has(Itm.gadget_fly.value, player) and state.has_any(
        [Itm.gadget_net.value, Itm.gadget_club.value], player
    )


## Quad Jump
def can_qj(state: CollectionState, player: int):
    return state.has(Itm.gadget_club.value, player) and state.has_any(
        [
            Itm.gadget_net.value,
            Itm.gadget_radar.value,
            Itm.gadget_hoop.value,
...
def can_glitch_float_morph(state: CollectionState, player: int):
    return state.has_any([Itm.morph_ninja.value, Itm.morph_monkey.value], player)
...
class Rulesets:
    ...
    def __init__(self, *rules: Callable | list[Callable] | list[list[Callable]] | None, critical: set[Callable] = None):
        self.critical = set()
        self.rules = []
...
class ProgressionMode:
    ...
    def generate_new_order(self, world: "AE3World") -> list[int]:
        ...
        if (
            len(set(new_order[:5]).intersection(self.small_starting_channels)) > 0
            or len(set(new_order[:3]).intersection([*self.boss_indices, *self.small_starting_channels])) > 0
        ):
    ...
    def reorder(self, set_interest: int, channels: list[str]):
        ...
        if 0 in self.progression[1:-1]:
            temp_progression = [
                self.progression[0],
                *[_ for _ in self.progression[1:-1] if _ > 0],
                self.progression[-1],
            ]
        ...
        targets: list[int] = [
            LEVELS_BY_ORDER.index(channel) for channel in channels if channel in LEVELS_BY_ORDER
        ].copy()
        ...
        for i, channel_set in enumerate(self.progression):
            ...
            group_set.append([_ if _ not in targets else -1 for _ in self.order[count:target]])
            count = target
        ...
        if additive:
            ...
            temp_order: list[int] = [channel for sets in group_set[:set_interest] for channel in sets if channel != -1]
        ...
        for i, channel_set in enumerate(self.progression):
            ...
            for channel in range(channel_set):
                ...
                if i == len(self.progression) - 2:
                    access_rule = Rulesets(
                        world.post_game_condition.enact(required_keys - 1, world.options.monkeysanity_break_rooms.value)
                    )
    ...
    def register_pgc_indirect_connections(self, world: "AE3World"):
        ...
        for channel in self.order[:main_channels_count]:
            ...
            if world.options.post_game_condition_monkeys:
    ...
    def regenerate_level_select_entrances(self):
        ...
        for slot, channel in enumerate(self.order):
            entrance: AE3EntranceMeta = AE3EntranceMeta(
                ENTRANCES_CHANNELS[slot], Stage.travel_station_a.value, base_destination_order[channel]
            )
...
class Group(ProgressionMode):
    ...
    def shuffle(self, world: "AE3World"):
        ...
        blacklist: list[int] = [
            LEVELS_BY_ORDER.index(channel)
            for channel in sorted(world.options.blacklist_channel)
    ...
    def __init__(self, world: "AE3World" = None):
        ...
        while sum(self.progression) < 27:
            ...
            if attempts > 5:
                self.progression = [3, *[4 for _ in range(6)], 0]
                warnings.warn(
                    "AE3 > Randomize: Generation failed to generate a valid Channel Set. "
...
ProgressionModeOptions: Sequence[Callable] = [Singles, Group, World, Quadruples, Open, Randomize]
