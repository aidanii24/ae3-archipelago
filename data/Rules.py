...
from .Locations import (
    ...
    CAMERAS_MASTER,
    CELLPHONES_INDEX,
    EVENTS_INFINITE_GADGET_FLOAT_APPLICABLE,
    LOCATIONS_DIRECTORY,
    LOCATIONS_INDEX,
    MONKEYS_BOSSES,
    MONKEYS_BREAK_ROOMS,
    MONKEYS_INDEX,
    MONKEYS_INFINITE_GADGET_FLOAT_APPLICABLE,
    MONKEYS_MASTER,
    MONKEYS_PASSWORDS,
    SHOP_CHEAP_COLLECTION_INDEX,
    SHOP_CHEAP_COLLECTION_MASTER,
    SHOP_CHEAP_MASTER,
...
from .Logic import (
    ...
    can_access_region,
    can_farm_boxes,
    can_farm_sneaky_borgs,
    event_invoked,
    has_enough_keys,
    has_keys,
    has_morph_extensions,
    has_morph_stocks,
    has_shop_stock,
)
from .Stages import (
    ENTRANCES_INDEX,
    ENTRANCES_INFINITE_GADGET_FLOAT_APPLICABLE,
    ENTRANCES_SHOP_PROGRESSION,
...
from .Strings import APHelper, Events, Loc, Stage

if TYPE_CHECKING:
    from .. import AE3Options, AE3World
    ...
    def __init__(
        ...
        if locations_excluded:
            ...
            if len(self.locations) <= 0:
                ...
                raise OptionError(
                    "AE3 > GoalTarget: There are no goal locations available to check. "
                    "They might have been explicitly excluded. Please reduce the excluded locations.",
                    f"{actual}",
                    f"{excluded}",
                )
            if len(self.locations) < self.amount:
                warn(
                    "AE3 > GoalTarget :"
...
class PostGameCondition:
    ...
    location_categories: ClassVar[list[str]] = [
        APHelper.monkey.value,
    ...
    def verify(self, state: CollectionState, player: int, min_keys: int, break_rooms: int = 0) -> bool:
        ...
        if APHelper.camera.value in self.amounts:
            count: int = sum(
                [state.can_reach_location(camera, player) for camera in self.locations[APHelper.camera.value]]
            )
    ...
    def check(self, ctx: "AE3Context") -> bool:
        ...
        for category in self.location_categories:
            if category not in self.amounts or category not in self.location_ids:
                continue
    ...
    def get_progress(self, ctx: "AE3Context") -> dict[str, list[int]]:
        ...
        for category in self.location_categories:
            if category in self.amounts:
                progress[category] = [
                    len(total_checked.intersection(self.location_ids[category])),
                    self.amounts[category],
                ]

        if APHelper.keys.value in self.amounts:
            progress[APHelper.keys.value] = [
                ctx.keys,
    ...
    def get_remaining(self, ctx: "AE3Context") -> dict[str, list[str]]:
        ...
        for category in self.location_categories:
            if category in self.amounts:
                remaining[category] = [
                    location for location in self.locations[category] if generate_name_to_id()[location] not in checked
                ]
...
class LogicPreference:
    ...
    def get_channel_clear_rules(self, channel: str) -> Rulesets:
        ...
        if channel not in STAGES_DIRECTORY:
            return rules
...
class Hard(LogicPreference):
    ...
    def __init__(self):
        ...
        self.monkey_rules.update(
            {
                ...
                Loc.castle_monga.value: Rulesets(
                    AccessRule.SHOOT, AccessRule.GLIDE, AccessRule.KUNGFU, AccessRule.MAGICIAN
                ),
                # Studio
                ...
                # Asia
                Loc.asia_baku.value: Rulesets(AccessRule.NINJA, AccessRule.HERO, [AccessRule.SHOOT, AccessRule.SWIM]),
                Loc.asia_mohcha.value: Rulesets(AccessRule.RCC, AccessRule.MONKEY),
                Loc.asia_takumon.value: Rulesets(
                    AccessRule.SWIM,
                    AccessRule.CATCH_LONG,
        ...
        self.event_rules.update(
            {
                # Studio
                Events.studio_b1_button.value: Rulesets(AccessRule.SHOOT, AccessRule.GLIDE, AccessRule.MAGICIAN),
                # Halloween
                Events.halloween_b1_jumbo_robo_shoot.value: Rulesets(AccessRule.SHOOT),
                # Edotown
                Events.edotown_e_scroll.value: Rulesets(AccessRule.CLUB, AccessRule.HOOP, AccessRule.MORPH),
                # Iceland
                Events.iceland_e_button.value: Rulesets(AccessRule.SLING),
                # Arabian
                Events.arabian_c_golden_mon.value: Rulesets(AccessRule.CATCH),
                # Asia
                Events.asia_e1_button.value: Rulesets(AccessRule.RCC, AccessRule.SHOOT, critical={AccessRule.FLY}),
                # Hong
        ...
        self.entrance_rules.update(
            {
                ...
                Stage.entrance_onsen_b1b.value: Rulesets(
                    AccessRule.GLIDE, AccessRule.KUNGFU, [AccessRule.RCC, AccessRule.SWIM]
                ),
                ...
                Stage.entrance_edotown_b2b1.value: Rulesets(
                    [event_invoked(Events.edotown_b1_button.value), AccessRule.NINJA], AccessRule.SWIM, AccessRule.HERO
                ),
                ...
                # Heaven
                Stage.entrance_heaven_ab.value: Rulesets(AccessRule.GLIDE, AccessRule.KUNGFU, AccessRule.MAGICIAN),
                ...
                Stage.entrance_asia_a1a3.value: Rulesets(
                    AccessRule.SWIM, [AccessRule.HERO, event_invoked(Events.asia_a2_block.value)]
                ),
                ...
                Stage.entrance_asia_a3e.value: Rulesets(
                    [
                        event_invoked(Events.asia_a_block.value),
                        event_invoked(Events.asia_a1_block.value),
                        event_invoked(Events.asia_a2_block.value),
                ...
                Stage.entrance_asia_bb2.value: Rulesets(AccessRule.HERO, event_invoked(Events.asia_b2_button.value)),
                ...
                Stage.entrance_asia_e1e2.value: Rulesets(
                    AccessRule.SHOOT, [AccessRule.ATTACK, AccessRule.RCC], critical={AccessRule.FLY}
                ),
                Stage.entrance_asia_e2e.value: Rulesets([event_invoked(Events.asia_e1_button.value), AccessRule.SWIM]),
                # Plane
                ...
                # Tomo
                Stage.entrance_tomo_a1a.value: Rulesets(AccessRule.GLIDE, AccessRule.MAGICIAN, AccessRule.KUNGFU),
                ...
                Stage.entrance_space_ib.value: Rulesets(
                    [
                        event_invoked(Events.space_e_button.value),
                        event_invoked(Events.space_f2_button.value),
                        event_invoked(Events.space_g1_button.value),
...
class Expert(Hard):
    ...
    def __init__(self):
        ...
        self.monkey_rules.update(
            {
                ...
                Loc.toyhouse_mokitani.value: Rulesets(AccessRule.RCC, AccessRule.FLYER, AccessRule.NINJA),
                Loc.snowfesta_mitsuro.value: Rulesets(
                    AccessRule.SHOOT, AccessRule.QJ, AccessRule.G_FLOAT, AccessRule.G_FLOAT_M
                ),
                Loc.asia_baku.value: Rulesets(
                    [AccessRule.SHOOT, AccessRule.SWIM], AccessRule.NINJA, AccessRule.HERO, AccessRule.KUNGFU
                ),
                Loc.asia_takumon.value: Rulesets(
                    AccessRule.SWIM, AccessRule.NINJA, AccessRule.COWBOY, AccessRule.HERO, AccessRule.KUNGFU
                ),
                Loc.asia_ukki_ether.value: Rulesets(AccessRule.SWIM, AccessRule.HERO, AccessRule.KUNGFU),
                ...
                Loc.hong_bankan.value: Rulesets(AccessRule.GLIDE, AccessRule.KUNGFU),
                Loc.hong_sukei.value: Rulesets(AccessRule.SHOOT, AccessRule.FLY),
                Loc.tomo_riley.value: Rulesets([AccessRule.KUNGFU, AccessRule.MAGICIAN], AccessRule.MAGICIAN),
                Loc.space_rokkun.value: Rulesets(AccessRule.RCC, AccessRule.COWBOY),
            }
        )
        ...
        self.event_rules.update(
            {
                ...
                Events.asia_e1_button.value: Rulesets(
                    [AccessRule.FLY, AccessRule.SHOOT],
                    [AccessRule.FLY, AccessRule.RCC],
        ...
        self.entrance_rules.update(
            {
                Stage.entrance_seaside_ac.value: Rulesets(AccessRule.MONKEY, AccessRule.KUNGFU),
                Stage.entrance_castle_aa2.value: Rulesets(
                    event_invoked(Events.castle_a2_button.value), AccessRule.KUNGFU
                ),
                ...
                Stage.entrance_onsen_be.value: Rulesets(
                    AccessRule.FLY, AccessRule.QJ, AccessRule.G_FLOAT, AccessRule.G_FLOAT_M
                ),
                ...
                Stage.entrance_snowfesta_dg.value: Rulesets(
                    AccessRule.MONKEY, AccessRule.KUNGFU, [AccessRule.QJ, AccessRule.G_FLOAT]
                ),
                Stage.entrance_edotown_be.value: Rulesets(
                    event_invoked(Events.edotown_e_scroll.value), AccessRule.NINJA
                ),
                Stage.entrance_edotown_c1c.value: Rulesets(
                    AccessRule.NINJA, AccessRule.HERO, AccessRule.KUNGFU, AccessRule.QJ, AccessRule.G_FLOAT_M
                ),
                ...
                Stage.entrance_arabian_e1e.value: Rulesets(AccessRule.MAGICIAN, AccessRule.FLY),
                Stage.entrance_asia_ab.value: Rulesets(AccessRule.SWIM, AccessRule.HERO, AccessRule.KUNGFU),
                Stage.entrance_asia_ba.value: Rulesets(AccessRule.SWIM, AccessRule.HERO, AccessRule.KUNGFU),
                ...
                Stage.entrance_asia_a2a4.value: Rulesets(AccessRule.SWIM, AccessRule.HERO, AccessRule.KUNGFU),
                Stage.entrance_asia_a3a4.value: Rulesets(
                    AccessRule.SWIM,
                    [event_invoked(Events.asia_a2_block.value), AccessRule.HERO],
                    [event_invoked(Events.asia_a2_block.value), AccessRule.KUNGFU],
                ...
                Stage.entrance_plane_cg.value: Rulesets(AccessRule.MONKEY, AccessRule.KUNGFU),
                Stage.entrance_plane_dd1.value: Rulesets(
                    AccessRule.GLIDE, AccessRule.MORPH, AccessRule.QJ, AccessRule.G_FLOAT, AccessRule.G_FLOAT_M
                ),
                Stage.entrance_plane_d1d.value: Rulesets(
                    AccessRule.GLIDE, AccessRule.KUNGFU, AccessRule.QJ, AccessRule.G_FLOAT, AccessRule.G_FLOAT_M
                ),
                ...
                Stage.entrance_hong_b1f.value: Rulesets(AccessRule.MONKEY, AccessRule.KUNGFU, AccessRule.HERO),
                Stage.entrance_hong_bb1.value: Rulesets(
                    AccessRule.KUNGFU, AccessRule.BOOST_FLY, AccessRule.QJ, AccessRule.G_FLOAT, AccessRule.G_FLOAT_M
                ),
                ...
                Stage.entrance_hong_cc2.value: Rulesets(AccessRule.FLYER, AccessRule.QJ, AccessRule.G_FLOAT),
                ...
                Stage.entrance_bay_aa1.value: Rulesets(
                    ...
                    AccessRule.MAGICIAN,
                    AccessRule.KUNGFU,
                    [AccessRule.SWIM, AccessRule.QJ],
                    [AccessRule.SWIM, AccessRule.G_FLOAT],
                    AccessRule.G_FLOAT_M,
                ),
                ...
                Stage.entrance_bay_a6f.value: Rulesets(
                    AccessRule.MONKEY, AccessRule.KUNGFU, [AccessRule.SWIM, AccessRule.FLYER]
                ),
                Stage.entrance_bay_d1d.value: Rulesets(
                    AccessRule.KUNGFU, AccessRule.BOOST_FLY, AccessRule.G_FLOAT, AccessRule.G_FLOAT_M
                ),
                Stage.entrance_bay_ee2.value: Rulesets(
                    AccessRule.FLYER,
                    AccessRule.NINJA,
                    AccessRule.QJ,
                    [AccessRule.G_FLOAT, AccessRule.CLUB],
                ...
                Stage.entrance_tomo_f1f2.value: Rulesets(
                    AccessRule.NINJA, AccessRule.HERO, AccessRule.KUNGFU, AccessRule.G_FLOAT_M
                ),
                ...
                Stage.entrance_space_ff1.value: Rulesets(
                    AccessRule.GLIDE, AccessRule.MORPH, AccessRule.BOOST_JUMP, AccessRule.QJ, AccessRule.G_FLOAT
                ),
            }
        )
    ...
    def apply_timed_kungfu_rule(self, base_duration: int, morph_extensions_enabled: bool):
        locations: list = [Loc.edotown_monkibeth.value]
        ...
        self.entrance_rules.get(Stage.entrance_asia_e1e2.value, Rulesets()).update(
            Rulesets(
                [AccessRule.SHOOT, AccessRule.SWIM, *rules],
    ...
    def apply_timed_morph_float(self, base_duration: int, morph_extensions_enabled: bool):
        events: list = [Events.iceland_e_button.value]
        ...
        self.monkey_rules.update(
            {
                ...
                Loc.castle_monga.value: Rulesets(AccessRule.SHOOT, AccessRule.GLIDE, AccessRule.MAGICIAN),
                # Ciscocity
                ...
                Loc.halloween_bonbon.value: Rulesets(
                    AccessRule.HIT,
                    event_invoked(Events.halloween_b_jumbo_robo.value),
                    event_invoked(Events.halloween_b1_jumbo_robo_shoot.value),
                ),
                Loc.halloween_chichi.value: Rulesets(
                    AccessRule.HIT,
                    event_invoked(Events.halloween_b_jumbo_robo.value),
                    event_invoked(Events.halloween_b1_jumbo_robo_shoot.value),
                ),
        ...
        self.event_rules.update(
            {
                ...
                Events.edotown_e_scroll.value: Rulesets(
                    AccessRule.CLUB, AccessRule.KNIGHT, AccessRule.NINJA, AccessRule.MAGICIAN, AccessRule.KUNGFU
                ),
                # Heaven
        ...
        self.entrance_rules.update(
            {
                ...
                Stage.entrance_edotown_b2b1.value: Rulesets(
                    [event_invoked(Events.edotown_b1_button.value), AccessRule.NINJA], AccessRule.SWIM
                ),
                ...
                Stage.entrance_heaven_ab.value: Rulesets(
                    AccessRule.NINJA, AccessRule.HERO, [AccessRule.FLYER, AccessRule.HOOP]
                ),
...
LogicPreferenceOptions: list = [Casual, Normal, Hard, Expert]
...
class ShopItemRules:
    ...
    def __init__(self, world: "AE3World"):
        ...
        post_game_channels: list[str] = [
            LEVELS_BY_ORDER[channel] for channel in world.progression.order[post_game_start_index:post_game_end_index]
        ]
        ...
        if world.options.cheap_items_minimum_requirement:
            ...
            if world.options.cheap_items_minimum_requirement.value < 100:
                cheap_items_rule = has_keys(
                    math.ceil(required_keys * (world.options.cheap_items_minimum_requirement.value / 100))
                )
        ...
        if world.options.shoppingsanity.value == 3:
            ...
            reached_shop_progress = has_keys
            ...
            reached_shop_progress = has_shop_stock
        ...
        for i, entrance in enumerate(ENTRANCES_SHOP_PSEUDOREGIONS):
            ...
            elif math.floor(current_set / self.sets) > 0:
            ...
            if cheap_items_early_amount and not enough_cheap_items:
                cheap: list[str] = [
                    item for item in SHOP_PROGRESSION_DIRECTORY[entrance.destination] if item in SHOP_CHEAP_MASTER
                ]
                expensive: list[str] = [
                    item for item in SHOP_PROGRESSION_DIRECTORY[entrance.destination] if item not in SHOP_CHEAP_MASTER
                ]
                ...
                if len(self.cheap_early_items) + len(cheap) <= cheap_items_early_amount:
                    ...
                    self.entrances.append(
                        AE3EntranceMeta(entrance.name, Stage.travel_station_b.value, entrance.destination)
                    )
                else:
                    enough_cheap_items = True
                    self.entrances.append(
                        AE3EntranceMeta(entrance.name, Stage.region_shop_expensive.value, entrance.destination)
                    )
            else:
                self.entrances.append(
                    AE3EntranceMeta(entrance.name, Stage.region_shop_expensive.value, entrance.destination)
    ...
    def set_pgc_rules(self, world: "AE3World") -> None:
        if not world.options.shoppingsanity.value:
            return
...
class PlaySpike(GoalTarget):
    ...
    locations = {
        monkey for monkey in MONKEYS_MASTER if monkey != Loc.boss_tomoki.value and monkey not in MONKEYS_PASSWORDS
    }

    amount = 204

    def verify(self, state: CollectionState, player: int) -> bool:
        minimum_equipment: list[Callable] = [
            AccessRule.DASH,
            AccessRule.SWIM,
            AccessRule.SLING,
            AccessRule.RCC,
